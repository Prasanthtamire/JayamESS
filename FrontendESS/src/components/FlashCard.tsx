...
type Props = {
  ...
  message: string;
  buttonText: string;
  onClose: () => void;       // For auto-dismiss or cancel
  onConfirm?: () => void;    // For warning confirm
  showCancel?: boolean;      // Show Cancel button only for warnings
};
    ...
    <div className={`flash-card ${type} ${fadeOut ? "fade-out" : ""}`}>
      ...
      {type === "warning" ? (
        <div className="flash-btn-group">
          <button className="flash-btn warning-ok" onClick={onConfirm}>
            {buttonText || "Confirm"}
          </button>
          {showCancel && (
            <button className="flash-btn warning-cancel" onClick={onClose}>
              Cancel
            </button>
          )}
        </div>
